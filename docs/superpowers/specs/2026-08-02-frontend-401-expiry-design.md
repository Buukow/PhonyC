...
- An eligible 401 clears `phonyg_token`, shows one browser alert with `登录状态已过期，请重新登录`, and navigates to `/login` using SPA history navigation so the router renders the login page without a full reload.
