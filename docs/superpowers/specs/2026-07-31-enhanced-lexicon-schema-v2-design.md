...
- Startup tests verify successful migration is persisted and logs removed fields, added fields, and schema-version change; invalid stored JSON remains unchanged, and every runtime generation path validates and falls back safely without panic or partial generation.
