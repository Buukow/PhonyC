      ...
      <form className="space-y-4" onSubmit={save}>
        ...
        {view === 'visual' ? (
          <div className="space-y-3">
            ...
            <div className="rounded-2xl border border-gray-100 p-3 space-y-2"><div className="font-medium text-sm text-gray-700">删除 Headers</div><Input value={doc.remove_headers.join(', ')} onChange={(e) => { const next = { ...doc, remove_headers: e.target.value.split(',').map((v) => v.trim()).filter(Boolean) }; setDoc(next); setJsonText(JSON.stringify(next, null, 2)) }} placeholder="逗号分隔，例如 X-Legacy, X-Debug" /></div>
          </div>
