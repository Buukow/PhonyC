...
渠道列表的“测活”会对该渠道第一个启用的模型映射发请求。选择客户端预设时只改变请求 Header，不改变协议对应的测活路径和请求体。普通停用或启用渠道只记录结果；已经临时禁用的渠道在成功时恢复。

点击后当前渠道的按钮会显示旋转加载状态并暂时禁用，直到请求和状态刷新结束；其他渠道仍可继续编辑、启用或同时测活。
